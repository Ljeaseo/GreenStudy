...
<div class="container">
	...
	<div class="detail_box">
		...
		<div class="detail_box_content">
			<h3><span style="color:red;">F</span>G3</h3><input type="hidden" id="gi_name" value="FG3">
			<h5>$1,339</h5><input type="hidden" id="gi_price" value=1339>
...
<div class="table_review">
	...
	<c:forEach var="comment" items="${comment}">
		...
		<tr>
			...
			<td class="emailtd" id="delete_permission_comment">${comment.user_email}</td>
		...
		<tr>
				<td colspan = 5 style='letter-spacing:10px' id="page">
					<c:if test="${paging.prev}">
						<a href="/FG3?guitar_no=1&type=${paging.cri.type}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}" style="color:black;">◁</a>
					</c:if>

					<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
 						<a href="/FG3?guitar_no=1&type=${paging.cri.type}&pageNum=${num}&amount=${paging.cri.amount}" style="color:black;">${num}</a>
					</c:forEach>
					
					<c:if test="${paging.next}">
						<a href="/FG3?guitar_no=1&type=${paging.cri.type}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}" style="color:black;">▷</a>
					</c:if>
				</td>
			</tr>
	</tbody>	
...
<div class="table_qna">
	...
	<input id="delete_permission_qna" type="hidden" value="${qna.user_email}">
		<tr>
			...
			<tr>
				<td colspan = 5 style='letter-spacing:10px' id="qnapage">
					<c:if test="${qnapaging.qprev}">
						<a href="/FG3?guitar_no=1&qtype=${qnapaging.qcri.qtype}&qpageNum=${qnapaging.qstartPage-1}&qamount=${qnapaging.qcri.qamount}" style="color:black;">◁</a>
					</c:if>

					<c:forEach begin="${qnapaging.qstartPage}" end="${qnapaging.qendPage}" var="num">
 						<a href="/FG3?guitar_no=1&qtype=${qnapaging.qcri.qtype}&qpageNum=${num}&qamount=${qnapaging.qcri.qamount}" style="color:black;">${num}</a>
					</c:forEach>
					
					<c:if test="${qnapaging.qnext}">
						<a href="/FG3?guitar_no=1&qtype=${qnapaging.qcri.qtype}&qpageNum=${qnapaging.qendPage+1}&qamount=${qnapaging.qcri.qamount}" style="color:black;">▷</a>
					</c:if>
				</td>
			</tr>	
		</tbody>	
