...
<div class="container">
	...
	<div class="detail_box">
		...
		<div class="detail_box_content">
			<h3>F<span style="color:red;">S</span>3</h3><input type="hidden" id="gi_name" value="FS3">
		...
		<tr>
				<td colspan = 5 style='letter-spacing:10px' id="page">
					<c:if test="${paging.prev}">
						<a href="/FS3?guitar_no=5&type=${paging.cri.type}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}" style="color:black;">이전</a>
					</c:if>

					<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
 						<a href="/FS3?guitar_no=5&type=${paging.cri.type}&pageNum=${num}&amount=${paging.cri.amount}" style="color:black;">${num}</a>
					</c:forEach>
					
					<c:if test="${paging.next}">
						<a href="/FS3?guitar_no=5&type=${paging.cri.type}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}" style="color:black;">다음</a>
					</c:if>
				</td>
			</tr>
	</tbody>	
			...
			<tr>
				<td colspan = 5 style='letter-spacing:10px' id="qnapage">
					<c:if test="${qnapaging.qprev}">
						<a href="/FS3?guitar_no=5&qtype=${qnapaging.qcri.qtype}&qpageNum=${qnapaging.qstartPage-1}&qamount=${qnapaging.qcri.qamount}" style="color:black;">이전</a>
					</c:if>

					<c:forEach begin="${qnapaging.qstartPage}" end="${qnapaging.qendPage}" var="num">
 						<a href="/FS3?guitar_no=5&qtype=${qnapaging.qcri.qtype}&qpageNum=${num}&qamount=${qnapaging.qcri.qamount}" style="color:black;">${num}</a>
					</c:forEach>
					
					<c:if test="${qnapaging.qnext}">
						<a href="/FS3?guitar_no=5&qtype=${qnapaging.qcri.qtype}&qpageNum=${qnapaging.qendPage+1}&qamount=${qnapaging.qcri.qamount}" style="color:black;">다음</a>
					</c:if>
				</td>
			</tr>	
		</tbody>	
